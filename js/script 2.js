...
var plumbStyle = {lineWidth:1,strokeStyle:'#818a89',dashstyle:" 2 4"}
var plumbConnect = [ "Flowchart", { cornerRadius: 40}]
...
function showFooter(){
	...
	TweenMax.to($('footer'), 1, {css:{className:"+=show"}, delay:1.5});
}
function hideFooter(){
	
	TweenMax.to($('footer'), 1, {css:{className:"-=show"}});
}
...
function updatebubbles(index){
	...
	$( "#fp-nav .progressbar" ).height( 43*(index-1) );
	$( "#fp-nav ul li a span" ).each(function( i ) {
...
$(function(){
    ...
    if($(window).scrollTop()==0){
     ...
     $('header .thematique').removeClass('hide');
    }else{
    $('header .thematique').addClass('hide');
	...
	$('.fullpage').fullpage({
		...
		afterSlideLoad: function(){ setTabs();},
